...
export function ProductScreen({ navigation, route }: Props): React.JSX.Element {
  ...
  const handleShare = useCallback(async () => {
    try {
        await Share.share({
          ...
          url: furniture.navigate_url ?? undefined,
        });
      } catch (error) {
  ...
  const handleBuyNow = useCallback(async () => {
    if (!furniture.navigate_url) return;
    
    const canOpen = await Linking.canOpenURL(furniture.navigate_url);
    if (canOpen) {
      await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      await Linking.openURL(furniture.navigate_url);
    }
  }, [furniture.navigate_url]);
